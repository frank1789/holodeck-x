  ...
  while (running_) {
    {
      ...
      //   &dock_id_left, &dock_id_main); ImGuiID dock_id_left_top = 0; ImGuiID
      //   dock_id_left_bottom = 0; ImGui::DockBuilderSplitNode(dock_id_left,
      //   ImGuiDir_Up, 0.50f, &dock_id_left_top, &dock_id_left_bottom);
      ...
      // ImGuiDockNodeFlags_PassthruCentralNode); ImGui::Begin("Properties");

      /////
      // ImGui::DockBuilderDockWindow("Main", dock_id_main);
      //       #include "imgui.h"
      // #include "imgui_internal.h" // Required for the advanced DockBuilder
      // API

      // void RenderUI()
      // {
      // 1. Setup a Fullscreen Dockspace
      // This allows the docking system to fill the entire application
      // workspace.
      ImGuiViewport* viewport = ImGui::GetMainViewport();
      ...
      // Hidden window flags that turn this window into an invisible background
      // hub
      ...
      // Programmatically Define the Default Layout
      // The conditional check ensures we only enforce this layout the VERY
      // first time the app runs. Afterwards, ImGui loads the user's custom
      // changes from the imgui.ini file.
      render_layout(false);
      ...
      // //   ImGui::DockBuilderSplitNode(dock_id_main, ImGuiDir_Left, 0.25f,
      // //                               &dock_id_left, &dock_id_main);

      // //   // Split Bottom: takes 30% of the height from the REMAINING main
      // node
      // //   ImGui::DockBuilderSplitNode(dock_id_main, ImGuiDir_Down, 0.30f,
      // //                               &dock_id_bottom, &dock_id_main);

      // //   // FIX: Split the bottom node 50/50 (0.5f) so BOTH tabs get equal
      // width
      // //   // space side-by-side
      // //   ImGui::DockBuilderSplitNode(dock_id_bottom, ImGuiDir_Left, 0.50f,
      // //                               &dock_id_log, &dock_id_cmd);

      // //   // Map your window titles onto the generated Dock IDs
      // //   ImGui::DockBuilderDockWindow("Inspector", dock_id_left);

      // //   // If you want "Console Output" stacked as a tab with "Ouput",
      // send it
      // //   // to dock_id_log:
      // //   ImGui::DockBuilderDockWindow("Console Output", dock_id_log);
      // //   ImGui::DockBuilderDockWindow(
      // //       "Ouput", dock_id_log);  // Note: Check if you meant "Output"
      ...
      // // ImGui will catch these by name and automatically snap them into
      // // position.

      // ImGui::Begin("Analysis Tree");
      // ImGui::Text("Entity Properties");
      // ImGui::End();

      // ImGui::Begin("Group & Selection");
      // ImGui::Text("Entity Properties");
      // ImGui::End();
      ...
      // // 1. Check if the mouse is hovering specifically over this active
      // window
      // // frame and if the user right-clicks (ImGuiMouseButton_Right)
      ...
      // // 4. Draw directly to the window's draw list (Bypasses
      // cursor/boundaries) ImDrawList* draw_list = ImGui::GetWindowDrawList();

      // // Optional: Draw a subtle dark background rectangle for readability
      // ImVec2 bg_min = ImVec2(text_pos.x - 4.0f, text_pos.y - 2.0f);
      // ImVec2 bg_max = ImVec2(text_pos.x + text_size.x + 4.0f,
      //                        text_pos.y + text_size.y + 2.0f);
      // draw_list->AddRectFilled(bg_min, bg_max, IM_COL32(0, 0, 0, 150), 4.0f);

      // // Draw the text
      // ImU32 text_color =
      //     ImGui::GetColorU32(ImGuiCol_TextDisabled);  // Muted grey
      ...
      // // renderer_->resize(w, h);                       // recreates the FBO
      // // only if the size changed renderer_->render(vs); // draws the scene
      // into
      // // the FBO
      ...
      // // Input handling is scoped to this widget — no more global mouse
      // state. const bool hovered = ImGui::IsItemHovered();
      // // if (hovered) {
      ...
      // //     camera.rotate_trackball(local, prev_local, Eigen::Vector2f(w,
      // h));
      ...
      // //     pick_at(renderer, local);                // GPU ID-buffer
      // readback,
      // //     see doc 05 §7
      // //   prev_local = local;
      // // }

      // // draw_axis_triad_overlay();                   // old w2, as an
      // // ImDrawList overlay draw_colorbar_overlay(vs);                   //
      // old
      // // scala_rgb/scala_tex
      // ImGui::PopStyleVar();
      // ImGui::End();
